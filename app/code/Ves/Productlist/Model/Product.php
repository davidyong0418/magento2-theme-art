...
class Product extends \Magento\Framework\DataObject
{
    ...
    public function getUnFeaturedProducts($config = [])
    {
        /** @var $collection \Magento\Catalog\Model\ResourceModel\Product\Collection */
        $collection = $this->_productCollectionFactory->create();
        if(isset($config['categories'])){
        ...
        $collection->setVisibility($this->_catalogProductVisibility->getVisibleInCatalogIds())
        ->addAttributeToSelect('*')
        ->addAttributeToFilter(array(array( 'attribute'=>'featured', 'eq' => '0')))
        ->addStoreFilter()
        ->setPageSize(isset($config['pagesize'])?$config['pagesize']:5)
        ->setCurPage(isset($config['curpage'])?$config['curpage']:1)
        ->getSelect()->group("e.entity_id");
    ...
    public function getProductBySource($source_key, $config = [])
    {
        ...
        switch ($source_key) {
            ...
            break;
            case 'except_featured':
            $collection = $this->getUnFeaturedProducts($config);
            break;
        }
        return $collection;
    }
